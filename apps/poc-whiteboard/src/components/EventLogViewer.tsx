...
const FILTER_OPTIONS: FilterConfig[] = [
  ...
  },
  {
    ...
    getEvents: (service) => service.getEventsByKindRange(NSM_PROTOCOL.INTERACTION_KIND_MIN, NSM_PROTOCOL.INTERACTION_KIND_MAX)
  }
];
...
export const EventLogViewer: React.FC<EventLogViewerProps> = ({
  ...
  useEffect(() => {
    const updateEvents = () => {
      ...
      if (filterConfig) {
        setEvents(filterConfig.getEvents(eventLogService));
      }
    };
    ...
    <div className={`bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col ${className}`}>
      ...
      <div className="border-b border-gray-200 p-4">
        ...
        <p className="text-sm text-gray-600 mt-1">
          Real-time monitoring of NSM framework events
        </p>
      </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-2">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getEventTypeColor(event.kind)}`}>
